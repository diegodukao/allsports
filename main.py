...
class Menu(Normal_screen):
	...
	def _save(self, *args):
		...
		try:
			...
			print(exc)
			self.label.text = 'Error saving database. Is it empty?'
			self.label.color = (1,0,0,1)
		else:
			self.label.text = 'Database successfuly saved'
	...
	def _load(self, *args):
		...
		try:
			...
			print(exc)
			self.label.text = 'Error loading database.'
			self.label.color = (1,0,0,1)
		else:
			self.label.text = 'Database successfuly loaded'
			self.label.color = (0,1,0,1)
	...
	def filechooser(self, save_flag, *args):
		...
		b_open = Button(text='Open', size_hint_x=0.1)
		...
		self.pp = Popup(title='Choose a folder', content=root)
		if save_flag:
