...
class List_games(Normal_screen):
	...
	def on_enter(self, **args):
		...
		with conn:
			...
			cur.execute('pragma foreign_keys = on;')
			cur.execute('create table if not exists games(id integer primary key autoincrement, team_name varchar(100), opponent_name varchar(100), date varchar(10), open integer, foreign key(team_name) references teams(name) on delete cascade on update cascade);')
			cur.execute('select opponent_name, date, open from games where team_name=? order by id desc;', (self.team_name,))
			games = cur.fetchall()
			if len(games):
				for game in games:
					b_game = List_btn(text='%s | %s' % (game[0], game[1]))
					if game[2]:
						b_game.bind(on_press=partial(self._go_open_game, b_game))
						b_game.background_color = (1, 1, 0, 1)
					else:	
						b_game.bind(on_press=partial(self._go_closed_game, b_game))
	...
	def _go_open_game(self, opponent_name, *args):
		opponent, _, date = opponent_name.text.partition(' | ')
		if not self.manager.has_screen('%s_%s_%s' % (self.team_name, opponent, date)):
			self.manager.add_widget(Open_game(self.team_name, opponent, date, name='%s_%s_%s' % (self.team_name, opponent, date)))
		self.manager.current = '%s_%s_%s' % (self.team_name, opponent, date)

	def _go_closed_game(self, opponent_name, *args):
		opponent, _, date = opponent_name.text.partition(' | ')
		if not self.manager.has_screen('%s_%s_%s' % (self.team_name, opponent, date)):
			self.manager.add_widget(Closed_game(self.team_name, opponent, date, name='%s_%s_%s' % (self.team_name, opponent, date)))
		self.manager.current = '%s_%s_%s' % (self.team_name, opponent, date)
...
class Closed_game(Normal_screen):
	...
	def _go_list_game_events(self, *args):
		if not self.manager.has_screen('%s_%s_%s_events' % (self.team_name, self.opponent_name, self.date_game)):
			self.manager.add_widget(List_game_events(self.team_name, self.opponent_name, self.date_game, name='%s_%s_%s_events' % (self.team_name, self.opponent_name, self.date_game)))
...
class Open_game(Normal_screen):
	def __init__(self, team_name, opponent_name, date_game, **args):
		...
		bl.add_widget(Title_lb(text='Running'))
		bl.add_widget(Title_lb(text= '%s vs %s' % (self.team_name, self.opponent_name)))
		bl.add_widget(Title_lb(text=date_game))
	...
	def _go_game_stats(self, *args):
		if not self.manager.has_screen('%s_%s_%s_stats' % (self.team_name, self.opponent_name, self.date_game)):
			self.manager.add_widget(Game_stats(self.team_name, self.opponent_name, self.date_game, name='%s_%s_%s_stats' % (self.team_name, self.opponent_name, self.date_game)))
	...
	def _confirm_popup(self, *args):
		...
		self.popup = Popup(title="Confirm you want to close this game", content=content, size_hint=(0.3, 0.3))
	...
	def close_game(self, *args):
		...
		with lite.connect('allsports.db') as conn:
			cur = conn.cursor()
			cur.execute('pragma foreign_keys = on;')
			cur.execute('update games set open=0 where team_name=? and opponent_name=? and date=? and open=1', (self.team_name, self.opponent_name, self.date_game))
		self.manager.current = self.team_name
		self.manager.remove_widget(self.manager.get_screen(self.name))
	...
	def _confirm_event(self, *args):
		if self.events_list.text!="Select an event" and self.players_list.text != "Select a player":
			with lite.connect('allsports.db') as conn:
				...
				cur.execute('insert into game_events(event_id, team_name, opponent_name, player_name, date_game) values(?, ?, ?, ?, ?);', (event_id, self.team_name, self.opponent_name, self.players_list.text, self.date_game))
			self.popup.dismiss()
...
class List_game_events(Normal_screen):
	...
	def on_enter(self, **args):
		...
		with conn:
			...
			try:
				cur.execute('create table if not exists game_events(id integer primary key autoincrement, event_id integer, team_name varchar(100), opponent_name varchar(100), player_name varchar(100), date_game varchar(10), time varchar(10), x integer, y integer, foreign key (event_id) references events(id) on update cascade on delete cascade, foreign key (player_name, team_name) references players(name, team_name) on update cascade on delete cascade);')
				cur.execute('select events.name, game_events.player_name from game_events join events on game_events.event_id=events.id where game_events.team_name=? and game_events.opponent_name=? and game_events.date_game=?;', (self.team_name, self.opponent_name, self.date_game))
				game_events = cur.fetchall()
				if len(game_events):
					for game_event in game_events:
						b_game_event = List_btn(text='%s | %s' % (game_event[0], game_event[1]))
