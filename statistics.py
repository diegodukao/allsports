...
class List_player_stats(Normal_screen):
		...
		with conn:
			...
			if len(self.list_games):
				for event in self.list_events:
					b_event = List_btn(text=event[0], background_color=(1,0,1,1))
					b_event.bind(on_press=partial(self._graph_popup, b_event.text))
					bl.add_widget(b_event)
			else:
				bl.add_widget(List_btn(text='There are no game records to show', background_color=(0,0,0,1)))
	...
	def _graph_popup(self, event_name,  *args):
		...
		if len(event_numbers) and max(event_numbers):
			try:
				graph = Graph(xlabel='Games', ylabel='Occurrences', x_ticks_major=1, y_ticks_major=1 ,y_grid_label=True , x_grid_label=True, padding=5, x_grid=True, y_grid=True,  xmin=1, xmax=len(event_numbers)+1, ymin=0, ymax=max(event_numbers)+5)
			...
			try:
				...
				if len(self.events):
					for game_event in self.events:
						b_game_event = List_btn(text='%s' % (game_event[0]), background_color=(1,0,1,1))
